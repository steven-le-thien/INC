...
INC and INC_NJ have been implemented in `python3` and the implementation is available in the `src/python` folder.
...
Various other changes (for example to the voting scheme), are available in the code, but not accessible from the command line. Don't be afraid to make changes as needed.
